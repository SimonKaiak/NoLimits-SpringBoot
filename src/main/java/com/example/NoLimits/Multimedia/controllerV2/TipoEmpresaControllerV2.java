...
public class TipoEmpresaControllerV2 {
    ...
    @PatchMapping("/{id}")
    @Operation(summary = "Actualizar parcialmente tipo de empresa (PATCH - HATEOAS)")
    public EntityModel<TipoEmpresaModel> patch(
            @PathVariable Long id,
            @RequestBody TipoEmpresaModel tipo
    ) {
        return assembler.toModel(service.patch(id, tipo));
    }
